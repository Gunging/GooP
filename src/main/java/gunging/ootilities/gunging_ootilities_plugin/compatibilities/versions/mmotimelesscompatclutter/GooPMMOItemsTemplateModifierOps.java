...
public class GooPMMOItemsTemplateModifierOps {
    ...
    public static Collection<TemplateModifier> getGlobalModifiers() {
        try {
            return (Collection<TemplateModifier>) globalModifiersList.invoke(MMOItems.plugin.getTemplates());
        } catch (IllegalAccessException|InvocationTargetException ignored) {
            return new ArrayList<>();
        }
    }

    public static Collection<TemplateModifier> getTemplateModifiers(@NotNull MMOItemTemplate template) {
        try {
            return ((Map<String, TemplateModifier>) templateModifiersList.invoke(template)).values();
        } catch (IllegalAccessException|InvocationTargetException ignored) {
            return new ArrayList<>();
        }
    }

    public static TemplateModifier getGlobalModifier(@NotNull String mod) {
        try {
            return (TemplateModifier) globalModifierGet.invoke(MMOItems.plugin.getTemplates(), mod);
        } catch (IllegalAccessException|InvocationTargetException ignored) {
            return null;
        }
    }

    public static TemplateModifier getTemplateModifier(@NotNull MMOItemTemplate template, @NotNull String mod) {
        try {
            return (TemplateModifier) templateModifierGet.invoke(template, mod);
        } catch (IllegalAccessException|InvocationTargetException ignored) {
            return null;
        }
    }
    ...
    public static ArrayList<String> getGlobalModifierNames() {
        ...
        for (TemplateModifier mod : getGlobalModifiers()) { ret.add(mod.getId()); }
    ...
    public static ItemStack ModifierOperation(@Nullable String rawModifier, @Nullable ItemStack mmo, boolean useGlobl, boolean chances, @Nullable RefSimulator<String> logger) {
        ...
        if (random) {
            ...
            // Shuffle
            List<TemplateModifier> modifiers = new ArrayList<>(getTemplateModifiers(template));
            ...
            // Get Modifier
            TemplateModifier modifier = modifierLocal ? getTemplateModifier(template, rawModifier) : getGlobalModifier(rawModifier);
