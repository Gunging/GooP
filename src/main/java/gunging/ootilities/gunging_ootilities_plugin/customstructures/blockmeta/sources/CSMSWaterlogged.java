...
public class CSMSWaterlogged extends CSMSBoolean {

    /**
     * This flag will have its corresponding {@link CSMBoolean} as
     * <code>true</code> if the thing is in 'waterlogged' state.
     */
    public CSMSWaterlogged() { super("WATERLOGGED"); }
    ...
    public Block apply(@NotNull Block block, @NotNull CSMBoolean meta, @NotNull Orientations inRelativeTo) {
        ...
        // Block data bisected??
        if (!(input instanceof Waterlogged)) { return block; }

        // Set the block data as the bisected half I guess
        ((Waterlogged) input).setWaterlogged(meta.getValue());
    ...
    public CSMBoolean fromBlock(@NotNull Block block, @NotNull Orientations inRelativeTo) {
        ...
        // Block data bisected??
        if (!(input instanceof Waterlogged)) { return null; }

        // Return data, where 'true' means its the bottom half.
        return new CSMBoolean((((Waterlogged) input).isWaterlogged()));
    }
    ...
    public boolean matches(@NotNull Block block, @NotNull CSMBoolean meta, @NotNull Orientations inRelativeTo) {
        ...
        if (!(input instanceof Waterlogged)) { return false; }

        // All right, well true means bottom
        return (meta.getValue() && ((Waterlogged) input).isWaterlogged()) || (!meta.getValue() && !((Waterlogged) input).isWaterlogged());
    }
}
